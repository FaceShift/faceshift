  ...
  for (let i = 0; i < faces.length; i++) {
    ...
    if (face.state === brfv4.BRFState.FACE_TRACKING_START ||
      ...
      if (preferences.getLeftClick() == InputOptions.mouth 
        || preferences.getRightClick() == InputOptions.mouth
        || preferences.getDoubleClick() == InputOptions.mouth) {
        ...
        if (mouthRet.mouth) {
          ...
          else if (preferences.getRightClick() == InputOptions.mouth) {
            //Right click
      ...
      let blinkStr = (`${preferences.getLeftClick()}${preferences.getRightClick()}${preferences.getDoubleClick()}`).toLowerCase();
      if (blinkStr.includes("blink")) {
        //Click if user has blinked
        if (prevWholeFace != []) {
          let blinkRet = blink.blinked(prevWholeFace, face.vertices,
            blinkStr.includes(InputOptions.leftblink.toLowerCase()),
            blinkStr.includes(InputOptions.rightblink.toLowerCase()));
          if (blinkRet.left && blinkRet.right) {
            ...
            else if (preferences.getLeftClick() == InputOptions.rightblink) {
              userClicked = blinkRet.waitingForTimeout || userClicked;
              if (blinkRet.right) {
                //If in drag mode, right blink will toggle mouse down/up.
                // else, right blink will left click mouse.
                if (preferences.getMode() == MouseModes.drag)
                  mouse.toggleBtnUpDwn();
                else
                  mouse.mouseLeftClick();
            ...
            else if (preferences.getRightClick() == InputOptions.leftblink) {
              userClicked = blinkRet.waitingForTimeout || userClicked;
              if (blinkRet.left)
                mouse.mouseRightClick();
            }
            if (preferences.getDoubleClick() == InputOptions.leftblink) {
              userClicked = blinkRet.waitingForTimeout || userClicked;
              if (blinkRet.left)
                mouse.mouseDoubleClick();
            }
            else if (preferences.getDoubleClick() == InputOptions.rightblink) {
              userClicked = blinkRet.waitingForTimeout || userClicked;
              if (blinkRet.right)
                mouse.mouseDoubleClick();
            }
      ...
      imageDataCtx.strokeStyle = userClicked ? "#e6e600" /*yellow*/ : "#00a0ff";
      /*blue*/

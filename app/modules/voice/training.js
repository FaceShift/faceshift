...
let recordCommands = (commandNames, commands) => {
    return new Promise(async (resolve, reject) => {
        let commandVoiceSamples = {}
        for (let i = 0; i < commandNames.length; i++) {
            let commandName = commandNames[i]
            let voiceSamples = await recordCommand(commandName, commands[commandName])
...
let recordCommand = (commandName, commandPhrase) => {
    return new Promise(async (resolve, reject) => {
        voiceSamples = []
